...
.stButton > button { background:var(--primary) !important; color:#FFFFFF !important; border:none !important; border-radius:8px !important; font-family:'Noto Sans KR',sans-serif !important; font-size:0.875rem !important; font-weight:600 !important; padding:0.6rem 1.25rem !important; transition:all 0.2s !important; }
...
# ============================
# [수정 1] 구글시트 로드 — 모든 컬럼 str 강제 변환 + 품번 4~9자리 필터
# ============================
...
NEGATIVE_KW = [
    ...
    "사지마","사지 마","추천안","추천 안","별1","뒤틀","휘어","금방 망가","오래 못가",
    "금방 부서","변질","변질됐","부서졌","터졌","녹았","녹이 슬음","녹이 슬었",
    "녹이 슬었어요","녹이 슬었네요","녹이 슨","녹이 슨거",
]
POSITIVE_KW = [
...
def is_promotional(item: dict) -> bool:
    ...
    return (sum(1 for kw in PROMO_KW if kw in full) >= 1
            and sum(1 for kw in NEGATIVE_KW if kw in full) == 0)
    ...
    if not best_sent:
        for kw in kw_list:
            ...
            if idx != -1:
                ...
                best_sent = full_text[start:end].strip()
                break
...
def parse_date(item: dict):
    ...
    # YYYYMMDD
    if re.fullmatch(r'\d{8}', str(ds)):
        try: return datetime.strptime(ds, "%Y%m%d")
        except: pass
    # RFC 2822 (카페 pubDate) — email.utils로 timezone 포함 안전 처리
    try:
        t = parsedate(ds)
...
def is_date_like(t: str) -> bool:
    ...
    if re.fullmatch(r'\d{6,8}', t): return True   # 6~8자리 연속 숫자 → 날짜 가능성
    return False
...
def match_product_name(code: str) -> str:
    ...
    for c in [c.strip() for c in code.split(",")]:
        ...
        if not row.empty: return str(row.iloc[0]["품명"])
    return ""


# ============================
# [수정 4] 엑셀 — '판단 근거 문장' 컬럼 추가
# ============================
def create_excel(data: list, start_dt: date, end_dt: date) -> io.BytesIO:
    ...
    for letter, width in zip("ABCDEFGHIJKLMNOP", [8,20,15,12,20,12,45,50,12,8,10,60,20,10,10,10]):
...
def render_result_card(r: dict):
    """결과 카드 렌더링 — [수정 3] 품목 태그 + [수정 4] 판단 근거 문장 포함"""
...
def sort_results(data: list, sort_key: str) -> list:
    if sort_key == "최신날짜순":
        return sorted(data, key=lambda x: x.get("날짜",""), reverse=True)
    if sort_key == "부정 높은순":
        return sorted(data, key=lambda x: (x["감성"]=="부정", x.get("확신도",0)), reverse=True)
    if sort_key == "긍정 높은순":
        return sorted(data, key=lambda x: (x["감성"]=="긍정", x.get("확신도",0)), reverse=True)
    if sort_key == "조회수 높은순":
        return sorted(data, key=lambda x: x.get("views") or 0, reverse=True)
    return data
...
<div class="app-header">
    <div style="display:flex;align-items:center;gap:0.5rem;flex-shrink:0;">
        <div style="width:48px;height:48px;background:#0066CC;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 6px rgba(0,102,204,0.35);">
    ...
    <div>
        ...
        <div class="header-sub">네이버 블로그 · 카페 · 유튜브 &nbsp;|&nbsp; KR-ELECTRA × KLUE-RoBERTa Ensemble</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    <div style="display:flex;align-items:center;gap:0.6rem;padding-bottom:1rem;border-bottom:1px solid #E2E8F0;margin-bottom:0.25rem;">
        <div style="width:32px;height:32px;background:#0066CC;border-radius:50%;display:flex;align-items:center;justify-content:center;">
            <span style="color:#FFFFFF;font-size:0.65rem;font-weight:900;font-family:'Inter',sans-serif;">D</span>
        </div>
        <div>
    ...
    st.markdown("""<div class="sb-section" style="margin:0.5rem 0 0.4rem;">
        <div class="sb-section-icon"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M4 11a9 9 0 0 1 9 9"/><path d="M4 4a16 16 0 0 1 16 16"/><circle cx="5" cy="19" r="1"/></svg></div>
        <span class="sb-section-text">CHANNEL</span>
    </div>""", unsafe_allow_html=True)
    r1l, r1r = st.columns(2)
    with r1l:
        c1, c2 = st.columns([1,4])
        with c1: search_blog = st.checkbox("", value=True, key="cb_blog", label_visibility="collapsed")
        with c2: st.markdown('<div class="ch-row"><div class="ch-icon ch-naver">N</div><span class="ch-label">블로그</span></div>', unsafe_allow_html=True)
    with r1r:
        c1, c2 = st.columns([1,4])
        with c1: search_cafe = st.checkbox("", value=True, key="cb_cafe", label_visibility="collapsed")
        with c2: st.markdown('<div class="ch-row"><div class="ch-icon ch-naver">N</div><span class="ch-label">카페</span></div>', unsafe_allow_html=True)
    r2l, r2r = st.columns(2)
    with r2l:
        c1, c2 = st.columns([1,4])
        with c1: search_yt = st.checkbox("", value=True, key="cb_yt", label_visibility="collapsed")
        with c2: st.markdown('<div class="ch-row"><div class="ch-icon ch-youtube"><svg width="9" height="9" viewBox="0 0 24 24" fill="#FFFFFF"><polygon points="5,3 19,12 5,21"/></svg></div><span class="ch-label">유튜브</span></div>', unsafe_allow_html=True)
    with r2r:
        st.markdown('<div class="ch-row" style="opacity:0.4;cursor:not-allowed;"><div style="width:20px;height:20px;border-radius:4px;background:#CBD5E1;display:flex;align-items:center;justify-content:center;font-size:0.55rem;color:#FFFFFF;font-weight:900;">N</div><span style="font-size:0.82rem;font-weight:500;color:#718096;text-decoration:line-through;">지식인</span></div>', unsafe_allow_html=True)

    # ② 검색어
    st.markdown("""<div class="sb-section" style="margin:0.5rem 0 0.3rem;">
        <div class="sb-section-icon"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div>
    ...
    # ③ 수집 기간
    st.markdown("""<div class="sb-section" style="margin:0.5rem 0 0.3rem;">
        <div class="sb-section-icon"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div>
    ...
    # ④ 수집 개수
    st.markdown("""<div class="sb-section" style="margin:0.5rem 0 0.3rem;">
        <div class="sb-section-icon"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg></div>
        <span class="sb-section-text">분석개수</span>
    </div>""", unsafe_allow_html=True)
    display_count = st.number_input("", min_value=10, max_value=5000, value=100, step=10,
                                    label_visibility="collapsed", help="데이터 수집건수 (최소 10 ~ 최대 5,000)")
    st.markdown('<span class="sb-hint">데이터 수집건수 · 10 ~ 5,000</span>', unsafe_allow_html=True)

    # ⑤ 감성 파라미터
    st.markdown("""<div class="sb-section" style="margin:0.5rem 0 0.3rem;">
        <div class="sb-section-icon"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg></div>
    ...
    <div class="param-guide-box">
        ...
        <b>📌 AI 모델 가중치</b><br>
        ELECTRA <code>×1.6</code> · RoBERTa <code>×1.0</code> · 룰베이스 <code>×0.8</code>
    </div>""", unsafe_allow_html=True)
...
if run_btn:
    ...
    total_tasks, done = len(collect_tasks), 0
    ...
    USIM_EXCLUDE_KW = [
        "유심","USIM","유심칩","유심카드","심카드","SIM카드","통신사","SKT","KT","LGU+",
        "알뜰폰","eSIM","이심","유심기변","유심교체","유심변경","해외유심","로밍유심",
        "글로벌유심","다이소유심","다이소 유심","다이소심카드","다이소 심카드",
    ]
    ...
    with tab_dash:
        ...
        for col, cls, lbl, val, pct, ic_txt in [
            ...
            with col:
                st.markdown(f'<div class="metric-card {cls}"><div class="metric-label"><span class="metric-icon {cls}" style="color:#FFFFFF !important;">{ic_txt}</span>{lbl}</div><div class="metric-value">{val}</div><div class="metric-pct">{pct}</div></div>', unsafe_allow_html=True)
        ...
        for col, lbl, val in [(d1,"소분류 수",str(sub_u)),(d2,"품번 수",str(code_u)),(d3,"품명 수",str(name_u))]:
            with col:
                st.markdown(f'<div class="card" style="text-align:center;padding:1rem 0.75rem;"><div style="font-size:1.6rem;font-weight:700;color:#0066CC;font-family:\'Inter\',sans-serif;">{val}</div><div style="font-size:0.72rem;color:#718096;margin-top:0.2rem;font-weight:500;">{lbl}</div></div>', unsafe_allow_html=True)
        ...
        if date_neg:
            ...
            st.altair_chart(
                alt.Chart(chart_df).mark_bar(color="#0066CC",cornerRadiusTopLeft=4,cornerRadiusTopRight=4)
                .encode(
        ...
        with col_top1:
            ...
            html = "".join([
                f'<div class="top-item"><div class="top-rank {"r1" if r==1 else ""}" style="color:{"#FFFFFF" if r==1 else "var(--primary)"};">{r}</div><div class="top-name">{n}</div><div class="top-count">{c}건</div></div>'
                for r,(n,c) in enumerate(sub_cnt.most_common(10),1)
            ])
        ...
        with col_top2:
            ...
            html2 = "".join([
                f'<div class="top-item"><div class="top-rank {"r1" if r==1 else ""}" style="color:{"#FFFFFF" if r==1 else "var(--primary)"};">{r}</div><div class="top-name">{n}</div><div class="top-count">{c}건</div></div>'
                for r,(n,c) in enumerate(code_cnt.most_common(10),1)
            ])
        ...
        st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1.25rem 0 0.75rem;">{icon("부정")} <span style="font-size:0.95rem;font-weight:600;">주요 부정 글 목록 (확신도 높은순)</span></div>', unsafe_allow_html=True)
        neg_top = sort_results([r for r in results if r["감성"]=="부정"], "부정 높은순")
        if neg_top:
            for r in neg_top[:20]:
    ...
    # ── 채널별 탭 공통 렌더러 ───────────────────────────────────
    def render_detail_tab(src_results: list, src_name: str, extra_sort_options: list = None):
        if not src_results:
        ...
        for col, cls, lbl, val, ic_txt in [
            ...
            with col:
                st.markdown(f'<div class="metric-card {cls}"><div class="metric-label"><span class="metric-icon {cls}" style="color:#FFFFFF !important;">{ic_txt}</span>{lbl}</div><div class="metric-value">{val}</div><div class="metric-pct">{round(int(val)/t*100) if t else 0}%</div></div>', unsafe_allow_html=True)
        ...
        for r in src_results:
            ...
            kw_stats[kw][r["감성"]] += 1
        kw_rows = [{"검색어":kw,"긍정":s["긍정"],"부정":s["부정"],"중립":s["중립"],
                    "합계":sum(s.values()),"부정률(%)":round(s["부정"]/sum(s.values())*100,1) if sum(s.values()) else 0}
        ...
        if extra_sort_options:
            sort_options += extra_sort_options
        ...
        sort_key = st.radio("정렬", sort_options, horizontal=True,
                            key=f"sort_{src_name}", label_visibility="collapsed")
    ...
    with tab_yt:
        render_detail_tab(
            [r for r in results if r["출처"]=="유튜브"],
            "유튜브",
            extra_sort_options=["조회수 높은순"]
        )
        if YOUTUBE_API_KEY:
            st.markdown("""
            <div style="margin-top:1.5rem;padding:1.25rem 1.5rem;background:#F8FAFC;border:1.5px dashed #CBD5E1;border-radius:10px;text-align:center;">
                ...
                <div class="badge-coming" style="display:inline-flex;">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#64748B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                    추가 예정 기능입니다
                </div>
            </div>
            """, unsafe_allow_html=True)
