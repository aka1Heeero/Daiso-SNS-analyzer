...
def _fetch(task, display_count):
    ...
    if tp == "yt_comments":
        ...
        for v in yt_items:
            ...
            if not vid:
                continue
            cs = fetch_youtube_comments(vid, max_comments=100)
            if cs:
...
# ============================================== 상품 이슈 카드 랭킹
def _render_product_issue_ranking(results: list, top_n: int = 12):
    """크롤링 결과를 상품(품번)별로 그룹핑해 카드 그리드로 랭킹.
    - 부정 언급 많은 순으로 정렬 (없으면 총 언급 순)
    - 각 카드: 품명·카테고리·긍부중 카운트·부정률·주요 이슈 TOP3·채널별 언급
    """
    from collections import defaultdict
    if not results:
        return
    groups = defaultdict(lambda: {"pos": 0, "neg": 0, "neu": 0,
                                   "issues": Counter(), "sources": Counter(),
                                   "pname": "", "cat": ""})
    for r in results:
        code = (r.get("품번") or "").strip()
        pname = (r.get("품명") or "").strip()
        ...
        if s == "긍정":
            ...
            g["neu"] += 1
        iss = (r.get("issue") or "").strip()
        if iss:
            g["issues"][iss] += 1
        src = r.get("출처", "")
        if src:
            g["sources"][src] += 1
        if not g["pname"] and pname:
            g["pname"] = pname
        if not g["cat"] and r.get("소분류"):
            g["cat"] = r.get("소분류")

    # 품명DB 조인으로 pname·cat 보완
    if not PRODUCT_DB.empty:
        for key, g in groups.items():
            if g["pname"] and g["cat"]:
                continue
            row = PRODUCT_DB[PRODUCT_DB["품번"].astype(str).str.strip() == key]
            if not row.empty:
                if not g["pname"]:
                    g["pname"] = str(row.iloc[0].get("품명", "")).strip()
                if not g["cat"]:
                    g["cat"] = str(row.iloc[0].get("소분류", "")).strip()

    ranked = sorted(
        groups.items(),
        key=lambda kv: (kv[1]["neg"], kv[1]["pos"] + kv[1]["neg"] + kv[1]["neu"]),
        reverse=True,
    )[:top_n]
    ...
    st.markdown(
        '<div style="font-family:\'IBM Plex Mono\',monospace;font-size:0.72rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:0.08em;margin:1.5rem 0 0.75rem;padding-bottom:0.4rem;border-bottom:1px solid #E1E7F0;">이슈 상위 상품 <span style="color:#A0AEC0;font-weight:500;">· 부정 언급 순 상위 ' + str(len(ranked)) + '</span></div>',
        unsafe_allow_html=True,
    )

    for row_start in range(0, len(ranked), 3):
        cols = st.columns(3)
        for i, (key, g) in enumerate(ranked[row_start: row_start + 3]):
            with cols[i]:
                total = g["pos"] + g["neg"] + g["neu"]
                neg_pct = round(g["neg"] / max(total, 1) * 100)
                rank = row_start + i + 1
                pname = g["pname"] or "(상품명 없음)"
                cat = g["cat"] or "카테고리 미매칭"
                code_disp = key if key.isdigit() else "품번 미확인"

                issue_top = g["issues"].most_common(3)
                if issue_top:
                    issue_html = "".join(
                        f'<div style="display:flex;justify-content:space-between;align-items:center;padding:0.32rem 0;border-top:1px dashed #E1E7F0;font-size:0.88rem;">'
                        f'<span style="color:#DC2626;font-weight:600;">{i_name}</span>'
                        f'<span style="font-family:\'IBM Plex Mono\',monospace;color:#7A879E;font-size:0.82rem;">{i_cnt}</span>'
                        f'</div>'
                        for i_name, i_cnt in issue_top
                    )
                else:
                    issue_html = '<div style="padding:0.5rem 0;color:#A0AEC0;font-size:0.85rem;">이슈 미기재 (AI 라벨링 필요)</div>'

                src_html = " · ".join(
                    f'<span style="color:#4A5568;">{s} {c}</span>'
                    for s, c in g["sources"].most_common(4)
                )

                bar_color = "#DC2626" if neg_pct >= 50 else ("#B45309" if neg_pct >= 25 else "#16A34A")

                st.markdown(f"""
                <div class="card" style="padding:1rem 1.15rem;margin-bottom:0.6rem;position:relative;">
                    <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:0.45rem;">
                        <span style="font-family:'IBM Plex Mono',monospace;font-size:0.85rem;font-weight:700;color:#2563EB;letter-spacing:0.05em;">RANK {rank:02d}</span>
                        <span style="font-family:'IBM Plex Mono',monospace;font-size:0.78rem;color:#A0AEC0;">{code_disp}</span>
                    </div>
                    <div style="font-size:1.1rem;font-weight:700;color:#0F1B2D;line-height:1.35;margin-bottom:0.2rem;">{pname}</div>
                    <div style="font-size:0.82rem;color:#7A879E;font-family:'IBM Plex Mono',monospace;margin-bottom:0.65rem;text-transform:uppercase;letter-spacing:0.05em;">{cat}</div>
                    <div style="display:flex;gap:0.9rem;font-size:0.9rem;font-family:'IBM Plex Mono',monospace;margin-bottom:0.4rem;">
                        <span style="color:#DC2626;font-weight:700;">부정 {g["neg"]}</span>
                        <span style="color:#16A34A;font-weight:600;">긍정 {g["pos"]}</span>
                        <span style="color:#B45309;">중립 {g["neu"]}</span>
                        <span style="color:#7A879E;margin-left:auto;">총 {total}</span>
                    </div>
                    <div style="height:6px;background:#E2E8F0;border-radius:3px;overflow:hidden;margin-bottom:0.5rem;">
                        <div style="height:100%;width:{neg_pct}%;background:{bar_color};"></div>
                    </div>
                    <div style="font-size:0.75rem;color:#7A879E;font-family:'IBM Plex Mono',monospace;text-transform:uppercase;letter-spacing:0.05em;margin-bottom:0.2rem;">주요 이슈</div>
                    {issue_html}
                    <div style="font-size:0.75rem;color:#7A879E;margin-top:0.6rem;padding-top:0.5rem;border-top:1px solid #E1E7F0;font-family:'IBM Plex Mono',monospace;">{src_html}</div>
                </div>
                """, unsafe_allow_html=True)


# ============================================== 전체 AI 라벨링 바 (대시보드 상단)
def _render_bulk_ai_labeling_bar(results: list):
    """대시보드 상단에 전체 AI 라벨링 버튼을 렌더링.
    - 아직 상품·이슈 정보가 부실한 항목만 대상
    - 예상 AI 요청 수 미리 표시 (하드리밋 판단용)
    - 클릭 시 세션 결과 목록의 각 항목에 상품·이슈·감성 채워 넣음
    """
    if not results:
        return
    # 대상: goldset 미확정 + 품명/이슈 없음
    pending = [
        (i, r) for i, r in enumerate(results)
        if not r.get("goldset") and (not r.get("품명") or not r.get("issue"))
    ]
    if not pending:
        st.markdown("""
        <div class="card" style="padding:0.85rem 1rem;margin-bottom:0.75rem;background:#F0FDF4;border:1px solid #A7F3D0;">
            <div style="font-family:'IBM Plex Sans KR',sans-serif;font-size:0.88rem;color:#15803D;font-weight:600;">
                모든 결과에 상품·이슈 정보가 채워져 있습니다.
            </div>
        </div>
        """, unsafe_allow_html=True)
        return

    total_batches = (len(pending) + 19) // 20
    used = _gemini_usage_today()
    remaining_quota = max(GEMINI_DAILY_LIMIT - used, 0)
    will_exceed = total_batches > remaining_quota

    st.markdown(f"""
    <div class="card" style="padding:0.95rem 1.15rem;margin-bottom:0.75rem;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:1rem;flex-wrap:wrap;">
            <div>
                <div style="font-family:'IBM Plex Sans KR',sans-serif;font-size:0.98rem;font-weight:700;color:#0F1B2D;line-height:1.4;">
                    상품 · 이슈 미채움 <b style="color:#2563EB;">{len(pending)}건</b>
                </div>
                <div style="font-family:'IBM Plex Mono',monospace;font-size:0.78rem;color:#7A879E;margin-top:0.25rem;letter-spacing:0.02em;">
                    AI 요청 <b>{total_batches}회</b> 필요 · 오늘 남은 사용량 <b>{remaining_quota}회</b>
                </div>
            </div>
            <div style="font-family:'IBM Plex Mono',monospace;font-size:0.72rem;color:{'#DC2626' if will_exceed else '#16A34A'};font-weight:700;text-transform:uppercase;letter-spacing:0.05em;">
                {'한도 초과 예상' if will_exceed else '실행 가능'}
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with _bc1:
        clicked = st.button(
            f"전체 AI 라벨링 시작 ({len(pending)}건)",
            key="dash_bulk_ai_run",
            use_container_width=True,
            disabled=will_exceed,
        )
    with _bc2:
        if will_exceed:
            st.caption(f"오늘 남은 AI 사용량({remaining_quota}회)이 부족합니다. 내일 다시 시도하거나 관리자에게 문의하세요.")
        else:
            st.caption("클릭 후 몇 분 대기 · 완료 시 이 페이지의 상품 · 이슈 정보가 자동으로 채워집니다.")
    ...
    if clicked:
        ...
        for bi in range(0, len(pending), BATCH):
            ok2, _ = _gemini_can_call()
            if not ok2:
                break
            chunk = pending[bi:bi + BATCH]
            items_for_ai = []
            for local_idx, (orig_idx, r) in enumerate(chunk):
                text_for_ai = ((r.get("title", "") or "") + " \n " + (r.get("description", "") or ""))[:2500]
                items_for_ai.append({
                    "idx": local_idx,
                    "title": r.get("title", ""),
                    "description": text_for_ai,
                })
            ai_results = analyze_batch_with_gemini(items_for_ai, candidates_ai)
            for ai in ai_results:
                try:
                    local_idx = int(ai.get("idx"))
                except Exception:
                    continue
                if local_idx < 0 or local_idx >= len(chunk):
                    continue
                orig_idx, _ = chunk[local_idx]
                r = results[orig_idx]
                ai_label = (ai.get("label") or "").strip()
                if ai_label in ("긍정", "부정", "중립"):
                    r["감성"] = ai_label
                    r["확신도"] = max(int(r.get("확신도", 70) or 70), 85)
                    r["ai_확정"] = True
                if ai.get("품번") and not r.get("품번"):
                    r["품번"] = str(ai["품번"]).strip()
                if ai.get("품명") and not r.get("품명"):
                    r["품명"] = str(ai["품명"]).strip()
                if ai.get("카테고리") and not r.get("소분류"):
                    r["소분류"] = str(ai["카테고리"]).strip()
                if ai.get("issue"):
                    r["issue"] = str(ai["issue"]).strip()
                updated += 1
            processed += len(chunk)
            try:
                prog.progress(min(processed / len(pending), 1.0))
                status.markdown(
                    f'<span style="font-size:0.85rem;color:#7A879E;font-family:\'IBM Plex Mono\',monospace;">AI 라벨링 진행 · {processed} / {len(pending)}</span>',
                    unsafe_allow_html=True
                )
            except Exception:
                pass
        ...
        st.success(f"AI 라벨링 완료 · 처리 {processed} · 채움 {updated}건")
        st.rerun()
...
with st.sidebar:
    ...
    with row1_left:
        ...
        with cb_col:
            search_blog = st.checkbox("", value=False, key="cb_blog", label_visibility="collapsed")
        with icon_col:
    ...
    with row1_right:
        ...
        with cb_col2:
            search_cafe = st.checkbox("", value=False, key="cb_cafe", label_visibility="collapsed")
        with icon_col2:
            st.markdown("""<div class="ch-row">
                <div class="ch-icon ch-naver">N</div>
    ...
    with row2_left:
        ...
        with cb_col_kin:
            search_kin = st.checkbox("", value=False, key="cb_kin", label_visibility="collapsed")
        with icon_col_kin:
            st.markdown("""<div class="ch-row">
                <div class="ch-icon ch-naver">N</div>
...
if run_btn:
    ...
    # ── 크롤링 완료 직후 자동 Gemini 배치 처리 (관리자만 · 관련도 상위) ──
    # 목적: 상품 랭킹·이슈 매트릭스에 실질 데이터가 자동 축적되도록
    #      일반 사용자는 하드리밋 절약을 위해 룰베이스 결과 그대로 사용
    try:
        if st.session_state.get("admin_mode"):
            ok, _reason = _gemini_can_call()
        else:
            ok = False
        if ok:
            # 대상: 관련도 15점+ AND 아직 goldset 미확정 AND 품명/이슈 정보 부실
            auto_targets = [
                (i, r) for i, r in enumerate(results)
                if r.get("관련성", 0) >= 15
                and not r.get("goldset")
                and (not r.get("품명") or not r.get("issue"))
            ]
            if auto_targets:
                # 최대 40개까지 (2번 요청)
                auto_targets = auto_targets[:40]
                auto_status = st.empty()
                auto_prog = st.progress(0)
                # 품명DB 후보 상위 20
                candidates_ai = []
                if not PRODUCT_DB.empty:
                    for _, row in PRODUCT_DB.head(20).iterrows():
                ...
                BATCH_AI = 20
                for bi in range(0, len(auto_targets), BATCH_AI):
                    chunk = auto_targets[bi:bi + BATCH_AI]
                    items_for_ai = []
                    for local_idx, (orig_idx, r) in enumerate(chunk):
                        text_for_ai = ((r.get("title", "") or "") + " \n " + (r.get("description", "") or ""))[:2500]
                        items_for_ai.append({
                    ...
                    for ai in ai_results:
                        ...
                        orig_idx, _ = chunk[local_idx]
                        r = results[orig_idx]
                        # 라벨 반영 (제외는 결과에서 제거하지 말고 표시만)
                        ai_label = (ai.get("label") or "").strip()
                        if ai_label in ("긍정", "부정", "중립"):
                            r["감성"] = ai_label
                            r["확신도"] = max(int(r.get("확신도", 70) or 70), 85)
                            r["ai_확정"] = True
                        # 상품/이슈 채우기 (기존 값 없을 때만)
                        if ai.get("품번") and not r.get("품번"):
                            r["품번"] = str(ai["품번"]).strip()
                        if ai.get("품명") and not r.get("품명"):
                            r["품명"] = str(ai["품명"]).strip()
                        ...
                        if ai.get("issue"):
                            r["issue"] = str(ai["issue"]).strip()
                    done = min(bi + BATCH_AI, len(auto_targets))
                    try:
                        auto_prog.progress(done / len(auto_targets))
                        auto_status.markdown(
                            f'<span style="font-size:0.78rem;color:#7A879E;font-family:\'IBM Plex Mono\',monospace;">AI 자동 라벨링 {done} / {len(auto_targets)}</span>',
                            unsafe_allow_html=True
                        )
                    except Exception:
                        pass
                auto_prog.empty(); auto_status.empty()
    except Exception:
        pass
...
if "analysis_results" in st.session_state and st.session_state["analysis_results"]:
    ...
    with tab_dash:
        # ── 전체 AI 라벨링 버튼 (관리자 전용) ──
        if st.session_state.get("admin_mode"):
            _render_bulk_ai_labeling_bar(results)
        ...
        # ── 상품별 이슈 카드 랭킹 (실사용자용, 시각적) ──
        _render_product_issue_ranking(results, top_n=12)
