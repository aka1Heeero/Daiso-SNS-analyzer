...
.admin-panel {
    background: var(--bg-white); border: 2px solid var(--admin);
...
.admin-kw-tag {
    ...
    padding: 0.25rem 0.65rem; font-size: 0.76rem; font-weight: 600; margin: 0.2rem;
}
.admin-section {
...
.admin-login-modal {
    max-width: 340px; margin: 3rem auto; background: var(--bg-white);
    border: 2px solid var(--admin); border-radius: 16px; padding: 2rem 1.75rem;
...
.admin-panel .stTextInput input, .admin-panel .stTextArea textarea {
    border: 1.5px solid var(--admin-md) !important; border-radius: 8px !important;
}
.admin-panel .stTextInput input:focus, .admin-panel .stTextArea textarea:focus {
    border-color: var(--admin) !important;
    box-shadow: 0 0 0 3px rgba(124,58,237,0.12) !important;
}
.admin-panel .stButton > button, .admin-btn > button {
    background: var(--admin) !important; color: #FFFFFF !important;
...
.model-status-ok {
    ...
    padding: 0.6rem 1rem; font-size: 0.82rem; color: #16A34A; font-weight: 600;
    margin-bottom: 0.5rem;
}
.model-status-warn {
    display: flex; align-items: center; gap: 0.5rem;
    background: #FEFCE8; border: 1px solid #FDE68A; border-radius: 8px;
    padding: 0.6rem 1rem; font-size: 0.82rem; color: #CA8A04; font-weight: 600;
    margin-bottom: 0.5rem;
}
.model-status-err {
    display: flex; align-items: center; gap: 0.5rem;
    background: #FEF2F2; border: 1px solid #FCA5A5; border-radius: 8px;
    padding: 0.6rem 1rem; font-size: 0.82rem; color: #DC2626; font-weight: 600;
    margin-bottom: 0.5rem;
}
...
.date-warn-box {
    ...
    padding: 0.6rem 1rem; font-size: 0.8rem; color: #92400E; margin-bottom: 0.5rem;
}
</style>
...
if "ADMIN_PASSWORD" not in st.secrets:
    st.error(
        "🚨 **보안 오류**: `secrets`에 `ADMIN_PASSWORD`가 설정되지 않았습니다.\n\n"
        "- **로컬**: `.streamlit/secrets.toml`에 `ADMIN_PASSWORD = \"강력한비밀번호\"` 추가\n"
        "- **Streamlit Cloud**: 앱 Settings → Secrets에 동일하게 추가"
    )
    st.stop()
...
def load_keywords_from_sheet():
    try:
        ...
        st.warning(f"⚠ 키워드 시트 로드 실패 (기본값 사용): {e}")
...
def save_keywords_to_sheet():
    try:
        ...
        ws = _get_or_create_worksheet(sh, "keywords", ["type", "keyword", "updated_at"])
        ws.clear()
        ws.append_row(["type", "keyword", "updated_at"])
...
def load_excluded_urls_from_sheet():
    try:
        ...
        st.warning(f"⚠ 제외 URL 시트 로드 실패: {e}")
...
def admin_apply_keyword(kw_type: str, keyword: str, action: str = "add"):
    ...
    if action == "add":
        if kw in lst: return False, f"이미 등록된 키워드입니다: {kw}"
        lst.append(kw)
        log_msg = f"[추가] {label_map[kw_type]} → '{kw}'"
    else:
        if kw not in lst: return False, f"목록에 없는 키워드입니다: {kw}"
        lst.remove(kw)
...
def extract_evidence_sentence(text: str, sentiment: str, neg_kw: list, pos_kw: list) -> str:
    """감성 판정 근거가 된 핵심 문장 1개 추출 (엑셀 다운로드용)"""
    kw_list = neg_kw if sentiment == "부정" else pos_kw if sentiment == "긍정" else []
    if not kw_list or not text.strip():
        return ""
    clean = re.sub(r'<[^>]+>', '', text)
    ...
    if not best:
        return ""
    # 60자 초과 시 키워드 주변으로 자르기
    if len(best) > 60:
        for kw in kw_list:
            idx = best.find(kw)
            if idx != -1:
                s = max(0, idx - 12)
                e = min(len(best), idx + 48)
                best = ("…" if s > 0 else "") + best[s:e] + ("…" if e < len(best) else "")
                break
    return best
...
def search_youtube(query: str, max_results: int = 30) -> list:
    ...
    try:
        ...
        st.session_state["yt_api_error"] = f"네트워크 오류: {e}"
        return []
    if "error" in data:
        err    = data["error"]
        code   = err.get("code", "")
        ...
        if code == 403 and reason == "quotaExceeded":
            ...
            st.session_state["yt_api_error"] = f"YouTube API 오류 ({code}): {msg}"
        return []
...
# ============================================================
# ★★★ BUG FIX 1: 날짜 파싱
# 네이버 pubDate 전체 형식을 우선 시도 (기존 코드 ds[:16] 슬라이싱 문제 완전 제거)
# ============================================================
_PUBDATE_FORMATS = [
    "%a, %d %b %Y %H:%M:%S +0900",
    "%a, %d %b %Y %H:%M:%S +0000",
...
def parse_date(item: dict):
    ...
    if re.fullmatch(r'\d{8}', ds):
        try:
            ...
            return None
    # 카페·지식인: "Mon, 28 Apr 2025 09:00:00 +0900" 형식
...
# ============================================================
# ★★★ BUG FIX 2: filter_by_date
# 날짜 파싱 실패 시 무조건 탈락 → 날짜 없으면 통과(포함)로 변경
# 수집된 글이 날짜 필드 없이도 결과에 포함되도록 보장
# ============================================================
...
def create_excel(data: list, start_dt: date, end_dt: date) -> io.BytesIO:
    ...
    headers = ["출처","검색어","소분류","품번","품명","가격언급","제목","링크","날짜","감성","확신도(%)","근거문장","채널/카페명","조회수","좋아요","댓글수"]
    ws.append(headers)
    ...
    for ri, row in enumerate(data, 2):
        ws.append([row.get(k,"") for k in ["출처","검색어","소분류","품번","품명","가격언급","title","link","날짜","감성","확신도","근거문장","channel","views","likes","comments"]])
        s = row.get("감성","")
        if s in col_bg:
        ...
        # 근거문장 셀 스타일 (이탤릭 + 연한 회색)
        ev_cell = ws.cell(ri, 12)
        if ev_cell.value:
            ev_cell.font = openpyxl.styles.Font(italic=True, color="555555", name="Malgun Gothic", size=9)
            ev_cell.fill = openpyxl.styles.PatternFill(start_color="F8F9FB", end_color="F8F9FB", fill_type="solid")
    for letter, width in zip("ABCDEFGHIJKLMNOP", [8,20,15,15,20,12,45,50,12,8,10,40,20,10,10,10]):
...
if st.session_state["admin_mode"]:
    ...
    with adm_tab1:
        ...
        with ac1:
            new_kw_input = st.text_input("추가할 키워드", placeholder="예: 냄새나요  /  가성비 최고", key="admin_new_kw")
        with ac2:
            kw_type_sel = st.selectbox("키워드 유형", options=["neg", "pos", "promo", "exclude"],
                format_func=lambda x: {"neg":"🔴 부정","pos":"🟢 긍정","promo":"🟡 홍보 제외","exclude":"⛔ 제목 직접 제외"}[x],
                key="admin_kw_type")
        if st.button("✅ 키워드 추가 & AI 재학습 반영", key="admin_add_kw_btn", use_container_width=True):
            ...
            if ok:
                st.success(f"✅ {msg}")
                st.markdown('<span class="relearn-badge">🔄 AI 룰베이스 재학습 완료 + Sheets 저장 완료</span>', unsafe_allow_html=True)
            else:
                st.warning(f"⚠ {msg}")
    ...
    with adm_tab3:
        ...
        with col_a:
            ...
            st.markdown(f'<div>{pos_tags}</div>', unsafe_allow_html=True)
        with col_b:
...
<div class="app-header">
    <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/96/Daiso_logo.svg/320px-Daiso_logo.svg.png"
         style="height:38px;object-fit:contain;flex-shrink:0;" alt="DAISO"/>
    <div style="width:1px;height:34px;background:#E2E8F0;margin:0 0.5rem;flex-shrink:0;"></div>
    <div>
...
with st.sidebar:
    ...
    <div style="display:flex;align-items:center;gap:0.6rem;padding-bottom:1rem;border-bottom:1px solid #E2E8F0;margin-bottom:0.25rem;">
        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/96/Daiso_logo.svg/320px-Daiso_logo.svg.png"
             style="height:26px;object-fit:contain;" alt="DAISO"/>
        <div>
            <div style="font-weight:700;font-size:0.88rem;color:#1A202C;">SNS Issue Finder</div>
    ...
    # [아이콘][라벨][체크박스] 한 줄 배치
    for key, label, color, icon_html in [
        ("cb_blog", "블로그",  "#03C75A", '<span style="color:#FFFFFF;font-size:0.55rem;font-weight:900;">N</span>'),
        ("cb_kin",  "지식인",  "#03C75A", '<span style="color:#FFFFFF;font-size:0.55rem;font-weight:900;">N</span>'),
        ("cb_cafe", "카페",    "#03C75A", '<span style="color:#FFFFFF;font-size:0.55rem;font-weight:900;">N</span>'),
        ("cb_yt",   "유튜브",  "#FF0000", '<svg width="9" height="9" viewBox="0 0 24 24" fill="#FFFFFF"><polygon points="5,3 19,12 5,21"/></svg>'),
    ]:
        _i, _l, _c = st.columns([1, 5, 1])
        with _i:
            st.markdown(f'<div style="padding-top:6px"><div style="width:20px;height:20px;background:{color};border-radius:4px;display:flex;align-items:center;justify-content:center;">{icon_html}</div></div>', unsafe_allow_html=True)
        with _l:
            st.markdown(f'<div style="padding-top:8px;font-size:0.82rem;font-weight:500;color:#1A202C;">{label}</div>', unsafe_allow_html=True)
        with _c:
            globals()[f"search_{key[3:]}"] = st.checkbox("", value=True, key=key, label_visibility="collapsed")

    search_blog = st.session_state.get("cb_blog", True)
    search_kin  = st.session_state.get("cb_kin",  True)
    search_cafe = st.session_state.get("cb_cafe", True)
    search_yt   = st.session_state.get("cb_yt",   True)
    ...
    # ── 분석 기간 (★ BUG FIX: 기본 시작일을 1년 전으로 수정) ──
    st.markdown("""
    <div class="sb-section" style="margin:0.5rem 0 0.3rem;">
        <div class="sb-section-icon">
            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#FFFFFF" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    # ★★★ BUG FIX 3: 기본 시작일을 1년 전(오늘 기준)으로 설정
...
if run_btn:
    ...
    if len(model_failures) == 2:
        st.error("🚨 AI 모델 2개 모두 로드 실패 — 룰베이스만으로 판정합니다.")
    elif len(model_failures) == 1:
        st.warning(f"⚠ {model_failures[0]} 모델 로드 실패 — 나머지 모델 + 룰베이스로 분석합니다.")
    else:
        st.markdown('<div class="model-status-ok">✅ AI 앙상블 모델 정상 로드 완료</div>', unsafe_allow_html=True)

    _display_count = display_count
    _yt_api_key    = YOUTUBE_API_KEY
    ...
    def _fetch(task, count=_display_count):
        tp, kw, label = task
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        ...
        for fut in concurrent.futures.as_completed(futures):
            try:
                ...
                label, kw, items = "오류", "", []
                st.warning(f"⚠ 수집 중 오류: {e}")
            all_items.extend(items)
            collect_log.append(f"{label}/{len(items)}건")
            done_tasks += 1
    ...
    # ★★★ BUG FIX 적용: 날짜 파싱 실패 시 포함하는 filter_by_date 사용
    ...
    <div class="card" style="border-left:3px solid #0066CC;">
        <span style="font-size:0.85rem;color:#0066CC;font-weight:600;">
            ✅ 수집 완료 — 총 <strong>{len(filtered)}</strong>건{note_str}
        </span><br>
    ...
    for batch_start in range(0, total_f, BATCH):
        if st.session_state.get("analysis_stopped"):
            break
        ...
        for idx, (full, (src, item, title)) in enumerate(zip(texts, metas)):
            ...
            subcate   = extract_subcategory(full)
            evidence  = extract_evidence_sentence(full, sentiment, get_neg_kw(), get_pos_kw())
...
# 채널 상세 탭 공통 함수
# ============================================================
def render_detail_tab(src_results: list, src_name: str, start_dt: date, end_dt: date):
    if not src_results:
    ...
    for r in src_results:
        ...
        # 소분류/품번/품명 태그행
        _product_row = ""
        if r.get("소분류") or r.get("품번") or r.get("품명"):
            _tags = ""
            if r.get("소분류"):
                _tags += f'<span style="background:#E8F1FB;color:#0066CC;border:1px solid #CCE0F5;padding:2px 8px;border-radius:12px;font-size:0.72rem;font-weight:600;margin-right:4px;">🗂 {r["소분류"]}</span>'
            if r.get("품번"):
                _tags += f'<span style="background:#F0FDF4;color:#16A34A;border:1px solid #A7F3D0;padding:2px 8px;border-radius:12px;font-size:0.72rem;font-weight:600;margin-right:4px;">🔢 {r["품번"]}</span>'
            if r.get("품명"):
                _tags += f'<span style="background:#FEFCE8;color:#CA8A04;border:1px solid #FDE68A;padding:2px 8px;border-radius:12px;font-size:0.72rem;font-weight:600;margin-right:4px;">🏷 {r["품명"]}</span>'
            if r.get("가격언급"):
                _tags += f'<span style="background:#FEF2F2;color:#DC2626;border:1px solid #FCA5A5;padding:2px 8px;border-radius:12px;font-size:0.72rem;font-weight:600;margin-right:4px;">💰 {r["가격언급"]}</span>'
            _product_row = f'<div style="margin:0.3rem 0 0.2rem;">{_tags}</div>'

        # 근거 문장
        _evidence = r.get("근거문장","")
        _evidence_row = ""
        if _evidence:
            _ev_color  = {"부정":"#FEF2F2","긍정":"#F0FDF4","중립":"#F8F9FB"}.get(r["감성"],"#F8F9FB")
            _ev_border = {"부정":"#FCA5A5","긍정":"#A7F3D0","중립":"#E2E8F0"}.get(r["감성"],"#E2E8F0")
            _ev_icon   = {"부정":"🔴","긍정":"🟢","중립":"⚪"}.get(r["감성"],"💡")
            _evidence_row = (
                f'<div style="font-size:0.74rem;color:#374151;'
                f'background:{_ev_color};border-left:3px solid {_ev_border};'
                f'padding:0.35rem 0.6rem;border-radius:0 6px 6px 0;margin:0.3rem 0;">'
                f'{_ev_icon} <em>"{_evidence}"</em></div>'
            )
        ...
        st.markdown(
            ...
            + _product_row
            + _evidence_row +
            '<div class="result-meta" style="margin-top:0.3rem;">'
            f'<span>🔍 {r["검색어"]}</span>'
            f'<span>📅 {r["날짜"]}</span>'
            + _badge +
            '</div>'
...
if st.session_state.get("analysis_done") and st.session_state.get("analysis_results"):
    ...
    with tab_dash:
        ...
        selected_opt = st.radio("필터", filter_options, index=current_idx, horizontal=True,
                                label_visibility="collapsed", key="dash_radio")
        ...
        if filtered_results:
            for idx, r in enumerate(filtered_results[:30]):
                ...
                if item_key in st.session_state.get("excluded_items", {}):
                    continue
                ...
                if r["감성"] == "부정" and neg_found:
                    summary = f"부정 키워드 감지: {', '.join(neg_found[:3])}"
                elif r["감성"] == "긍정" and pos_found:
                    summary = f"긍정 키워드 감지: {', '.join(pos_found[:3])}"
                elif r["감성"] == "부정":
                    summary = "AI 모델 부정 판단"
                elif r["감성"] == "긍정":
                    summary = "AI 모델 긍정 판단"
                ...
                st.markdown(
                    ...
                    f'<span>🔍 {r["검색어"]}</span>'
                    f'<span>📅 {r["날짜"]}</span>'
                    f'<span style="color:#718096;font-size:0.72rem;">{summary}</span>'
                    + _badge +
                    '</div>'
                    '</div>',
                    unsafe_allow_html=True)
                ...
                if st.session_state["admin_mode"]:
                    with st.expander(f"🛡 관리자: 이 글 제외하기 (#{idx+1})", expanded=False):
                        ...
                        with excl_col1:
                            reason = st.text_input("제외 사유", key=f"exclude_reason_{idx}",
                        ...
                        with excl_col2:
                            ...
                            if st.button("제외 확정", key=f"exclude_btn_{idx}", use_container_width=True):
                                if reason.strip():
                                    ...
                                    save_excluded_url_to_sheet(item_key, reason.strip())
                                    st.session_state["admin_retrain_log"].append({
