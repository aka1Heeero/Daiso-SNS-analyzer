...
_secret_products = [p.strip() for p in _raw.split(",") if p.strip()]
_default = [
...
def is_complaint(text, brand, complaint_keywords):
...
@st.cache_resource
def load_model():
    return pipeline("sentiment-analysis", model="snunlp/KR-ELECTRA-SC", truncation=True, max_length=512)

DEFAULT_POSITIVE = ["꿀이었","강추","최고","완벽","대박","굿","좋았","만족","추천","좋아요","훌륭","짱","최애","득템","필수템","꿀템"]
DEFAULT_NEGATIVE = ["최악","환불","불량품","쓰레기","망했","최저","형편없","절대비추","비추","후회","실망","돈낭비","진짜별로"]
...
def classify_type(text, complaint_types):
...
STOPWORDS = {
    ...
    "해당","관련","같은","어떤","이런","저런","그런","모든","일부","전체",
    "보면","대한","한국","일본","중국","온라인","오프라인","매장","블로그","지식인"
}

def extract_product(text, brand, product_dict):
    ...
    while start <= display:
        ...
        try:
            res = requests.get(url, headers=headers,
                params={"query": query, "display": fetch, "start": start, "sort": "date"}, timeout=10)
...
def create_excel(data, query, start_date, end_date):
    ...
    ws2["A1"] = "태깅 방법"
    ws2["A1"].font = openpyxl.styles.Font(bold=True, size=13)
    for row in [[],["입력값","의미"],["1","실제 불만글 → AI 학습 사용"],["0","불만 아닌 글 → AI 학습 사용"],["빈칸","확인 안 함 → 미사용"],[],["목표","1+0 합쳐서 200개 이상 → 파인튜닝 가능"]]:
        ws2.append(row)
...
with st.sidebar:
    ...
    query = st.text_input("🔎 검색어", value="다이소 상품 불만")
    if query:
...
COMPLAINT_TYPES = dict(DEFAULT_COMPLAINT_TYPES)
if extra_complaints:
    COMPLAINT_TYPES["기타불만"] = extra_complaints
...
if run:
    ...
    with st.spinner("📡 수집 중..."):
        if do_blog:
            ...
            all_items += b; st.info(f"블로그 {len(b)}개")
        if do_kin:
            k = search_naver(query, "kin", display_count)
            all_items += k; st.info(f"지식인 {len(k)}개")
    ...
    for i, item in enumerate(filtered):
        ...
        if not is_complaint(text, brand, COMPLAINT_KEYWORDS):
            skipped += 1
            prog.progress((i+1)/len(filtered), text=f"1차 필터 중... ({i+1}/{len(filtered)})")
            continue
        senti, score = ai_sentiment(text, model)
        product = extract_product(text, brand, PRODUCT_DICT)
        ctype   = classify_type(text, COMPLAINT_TYPES)
        info    = extract_info(text)
        dt      = parse_date(item)
        results.append({
            "출처": item["출처"], "품명": product,
            "title": re.sub(r'<[^>]+>', '', item.get("title","")),
            "link": item.get("link",""), "날짜": dt.strftime("%Y-%m-%d") if dt else "",
    ...
    for idx, (t, c) in enumerate(Counter(all_types).most_common(6)):
    ...
    st.info("📌 엑셀의 **'불만여부(태깅)'** 컬럼: 불만글=**1**, 일반글=**0** → 200개 모이면 AI 파인튜닝 가능")
    excel_buf = create_excel(results, query, start_date, end_date)
    st.download_button("📥 엑셀 다운로드", data=excel_buf,
