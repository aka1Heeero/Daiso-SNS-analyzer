...
with st.sidebar:
    ...
    with row3_right:
        ...
        with icon_col_pp:
            st.markdown("""<div class="ch-row">
                <div style="width:20px;height:20px;border-radius:4px;background:#EF4444;display:flex;align-items:center;justify-content:center;font-size:0.6rem;color:#FFFFFF;font-weight:900;flex-shrink:0;">P</div>
                <span class="ch-label">뽐뿌</span>
            </div>""", unsafe_allow_html=True)
...
if run_btn:
    ...
    if model_r is None:
        st.warning("AI 모델 로드 실패 · 단어 규칙만으로 판정합니다.")
    else:
        st.markdown(
    ...
    for kw in keywords:
        ...
        if search_ppomppu:
            collect_tasks.append(("ppomppu", kw, "뽐뿌"))
...
if "analysis_results" in st.session_state and st.session_state["analysis_results"]:
    ...
    _tab_labels = ["대시보드", "블로그", "카페", "지식iN", "유튜브", "유튜브 댓글", "뽐뿌"]
    if st.session_state.get("admin_mode"):
    ...
    with tab_ppomppu:
        render_detail_tab([r for r in results if r["출처"]=="뽐뿌"], "뽐뿌", start_date, end_date)
