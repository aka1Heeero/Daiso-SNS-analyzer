...
def _load_css(filename: str):
    """앱 파일 기준 업로드해 폴더의 CSS 파일을 읽어 주입."""
    css_path = Path(__file__).parent.parent / "업로드해" / filename
...
def gemini_available() -> bool:
    ...
    return bool(GEMINI_API_KEY) and _get_gemini_model() is not None
...
def get_excluded_url_set() -> set:
    """excluded_urls 시트에 등록된 URL 집합 반환 (크롤링 필터용, 캐시)."""
...
def _render_admin_analytics(current_results: list):
    ...
    # ────────────────────── (1) 상품×이슈 매트릭스
    st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:0 0 0.75rem;">{icon("📦")} <span style="font-size:0.95rem;font-weight:600;">상품 × 이슈 매트릭스</span> <span style="font-size:0.7rem;color:#7A879E;">(정답셋 라벨링 기반)</span></div>', unsafe_allow_html=True)
    ...
    if not prod_issues:
        st.info("아직 정답셋에 상품 정보가 없습니다. 결과 카드에서 [🏷 정답셋 라벨링]으로 상품·이슈를 저장해 주세요.")
    else:
        # 총 건수 기준 정렬
        rows_sorted = sorted(
            prod_issues.items(),
    ...
    st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1rem 0 0.75rem;">{icon("🎯")} <span style="font-size:0.95rem;font-weight:600;">감성분석 정확도</span> <span style="font-size:0.7rem;color:#7A879E;">(현재 결과 중 정답셋과 일치하는 항목 기준)</span></div>', unsafe_allow_html=True)
    ...
    # ────────────────────── (3) 제외 학습 특징어 채택
    st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1rem 0 0.75rem;">{icon("🧠")} <span style="font-size:0.95rem;font-weight:600;">제외 학습 특징어</span> <span style="font-size:0.7rem;color:#7A879E;">(관리자가 제외 라벨링한 글에서 자동 추출된 후보)</span></div>', unsafe_allow_html=True)
    ...
    if not candidate_kws:
        st.caption("제외 라벨링이 부족하거나 유의미한 특징어가 추출되지 않았습니다. 결과 카드에서 [🚫 제외로 저장]을 활용해 주세요.")
    else:
        ...
        for i, kw in enumerate(candidate_kws[:20]):
            ...
            with col_a:
                st.markdown(f'<div style="padding:0.35rem 0;font-size:0.85rem;font-weight:600;color:#0F1B2D;">🔸 {kw}</div>', unsafe_allow_html=True)
            with col_b:
...
def _run_batch_ai_labeling(page_results: list, src_name: str, page: int):
    ...
    if not gemini_available():
        st.warning("Gemini API 키가 없거나 사용 불가입니다.")
        return
    items_for_ai = []
    ...
    with st.spinner(f"🤖 Gemini 배치 분석 중... ({len(items_for_ai)}건, 한 요청)"):
        results = analyze_batch_with_gemini(items_for_ai, candidates)
    if not results:
        st.warning("배치 분석 실패. 개별 카드의 [🤖 AI 분석]으로 시도해 보세요.")
        return
    # 각 결과를 세션에 반영
    saved = 0
    for res in results:
...
def _render_ai_labeler(item: dict, src_name: str, page: int, idx: int):
    ...
    with st.expander("🏷 정답셋 라벨링 (관리자)", expanded=False):
        c1, c2, c3 = st.columns([1.3, 1, 1])
        with c1:
            if st.button("🤖 AI로 상품·이슈 분석", key=f"{key_base}_run",
                         use_container_width=True, type="secondary"):
                if not gemini_available():
                    st.warning("Gemini API 키가 설정되지 않았거나 사용 불가입니다.")
                else:
        ...
        with c6:
            if st.button("💾 정답셋 저장", key=f"{key_base}_save", use_container_width=True):
        ...
        with c7:
            if st.button("🚫 제외로 저장", key=f"{key_base}_exclude", use_container_width=True, type="secondary"):
                append_goldset_to_sheet(
                    url=url,
                    title=item.get("title", ""),
                    label="제외",
...
def render_detail_tab(src_results, src_name, start_date, end_date):
    ...
    if st.session_state.get("admin_mode"):
        ...
        with _bt0:
            if st.button(f"🤖 페이지 일괄 AI 라벨링", key=f"bulk_ai_{src_name}_{current_page}", use_container_width=True, type="secondary", disabled=not gemini_available()):
    ...
    for idx, r in enumerate(page_results):
        ...
        _gs    = ('<span style="background:#FEF3C7;color:#92400E;padding:2px 6px;border-radius:3px;font-size:0.65rem;font-weight:700;letter-spacing:0.03em;">⭐ 정답셋 확정</span>') if r.get("goldset") else ""
        _issue = ('<span style="color:#2563EB;font-weight:600;">💡 ' + r["issue"] + '</span>') if r.get("issue") else ""
        _badge = '<span class="' + _b + '">' + r["감성"] + ' ' + fmt_score(r["확신도"]) + '</span>'
        _rel_score = r.get("관련성", 0)
        if _rel_score >= 99:
            _rel = ''  # 정답셋 확정은 별도 뱃지 있음
        elif _rel_score >= 15:
            _rel = f'<span style="background:#EFF4FF;color:#1D4ED8;padding:2px 6px;border-radius:3px;font-size:0.65rem;font-weight:700;">🎯 관련도 상 {_rel_score}</span>'
        elif _rel_score >= 8:
            _rel = f'<span style="background:#F1F5F9;color:#475569;padding:2px 6px;border-radius:3px;font-size:0.65rem;font-weight:600;">관련도 중 {_rel_score}</span>'
        else:
            _rel = f'<span style="background:#FEF2F2;color:#DC2626;padding:2px 6px;border-radius:3px;font-size:0.65rem;font-weight:600;">관련도 저 {_rel_score}</span>'
...
with st.sidebar:
    ...
    with row2_right:
        st.markdown("""<div class="ch-row" style="opacity:0.5;cursor:not-allowed;">
            <div style="width:20px;height:20px;border-radius:4px;background:#6366F1;display:flex;align-items:center;justify-content:center;font-size:0.55rem;color:#FFFFFF;font-weight:900;flex-shrink:0;">C</div>
            <span style="font-size:0.82rem;font-weight:500;color:#718096;line-height:1;">커뮤니티</span>
            <span style="font-size:0.6rem;color:#A0AEC0;margin-left:2px;">(추가중)</span>
        </div>""", unsafe_allow_html=True)
...
if "analysis_results" in st.session_state and st.session_state["analysis_results"]:
    ...
    with tab_dash:
        ...
        if neg_all_text.strip():
            st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1.25rem 0 0.5rem;">{icon("!")} <span style="font-size:0.95rem;font-weight:600;">부정 리뷰 핵심 단어</span></div>', unsafe_allow_html=True)
            with st.container():
        ...
        try:
            ...
            if issue_texts.strip():
                st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1.25rem 0 0.5rem;">{icon("♥")} <span style="font-size:0.95rem;font-weight:600;">정답셋 이슈 워드클라우드</span> <span style="font-size:0.7rem;color:#7A879E;">(관리자 라벨링 기반)</span></div>', unsafe_allow_html=True)
                st.markdown('<div class="card" style="padding:0.75rem;">', unsafe_allow_html=True)
    ...
    with tab_yt:
        ...
        if not yt_results:
            ...
            if yt_current_filter == "긍정":
                ...
                yt_results = [r for r in yt_results if r["감성"] == "부정"]
            ...
            st.markdown(f'<div style="display:flex;align-items:center;gap:0.5rem;margin:1.25rem 0 0.75rem;">{icon("영상")} <span style="font-size:0.95rem;font-weight:600;">영상 목록 ({len(yt_sorted)}건)</span></div>', unsafe_allow_html=True)
    
            PAGE_SIZE_YT = 20
            total_pages_yt = (len(yt_sorted) - 1) // PAGE_SIZE_YT + 1 if yt_sorted else 1
    ...
    if tab_admin is not None:
        with tab_admin:
            ...
            # ── 각 카테고리 뱃지 표시 ──
            _render_badges("제외 키워드", "🚫", sheet_kw.get("exclude", []), "#FEF2F2", "#DC2626")
            _render_badges("부정 키워드", "👎", sheet_kw.get("neg", []), "#FEF2F2", "#DC2626")
